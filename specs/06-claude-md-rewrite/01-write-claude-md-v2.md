---
phase: 6
task_id: 01-write-claude-md-v2
agent: planner (Haiku 可、内容は本仕様 + overview に集約)
estimated_minutes: 15
depends_on: []
---

# Task: CLAUDE.md を v2 体制で書き直す

## Goal
戦国要素を完全削除、新体制 (planner / reviewers / engineers + memory.md) を反映した CLAUDE.md を作る。Claude Code が auto-load する公式エントリポイント。

## Steps

1. 既存 CLAUDE.md は legacy/ 化する (Phase 7) ので、新規 CLAUDE.md を以下の内容で書く:
...
```bash
git add CLAUDE.md
git commit -m "docs(v2): rewrite CLAUDE.md for new orchestration model (planner + reviewers + engineers)"
```

## Verification
```bash
head -3 CLAUDE.md
# Expected: "# agent-orchestra-makoto-mizuno — CLAUDE.md"
grep -c "戦国\|家老\|足軽\|軍師" CLAUDE.md
# Expected: 0
```

## Notes
- 旧 CLAUDE.md は Phase 7 で legacy/ に移動してから削除
- 新 CLAUDE.md は user/project Memory で auto-load される (Claude Code 仕様)
- 戦国系単語 (将軍、家老、足軽、軍師、しゅっじん 等) ゼロ確認必須
