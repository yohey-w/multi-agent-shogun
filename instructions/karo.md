...
### Shogun への inbox_write（severity 付き）

殿への push は severity を必ず指定すること（5th arg）。

```bash
# critical — 即時配信（殿が入力中でも届く）
bash scripts/inbox_write.sh shogun "<message>" action_required karo critical

# info — 蓄積のみ（殿が入力中はスキップ、48h後自動read化）
bash scripts/inbox_write.sh shogun "<message>" report_received karo info
```
...
| Severity | 条件 | 例 |
|----------|------|----|
| `critical` | 即時判断が必要 / 実装がブロックされている / システム障害 | 「本番エラー多発」「決済不能」「承認なしで進めない設計判断」 |
| `info` | 定常的な完了報告 / レビュー結果 / 次回でも問題ない要対応 | 「cmd_XXX 完了」「軍師レビュー結果」「技術選定案の報告」 |

**迷ったら `info`**（デフォルト）。`critical` は「今すぐ殿の目に入らなければ損害が出る」場合のみ。

#### dashboard.md との併用

- inbox_write (severity付き) → 殿への通知
- dashboard.md 🚨 要対応 → 全アクション項目を一覧管理（変更なし）

どちらか片方だけではなく、両方を更新すること。
