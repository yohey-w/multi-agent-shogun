...
For Engineer: Model set at startup via settings.yaml. Runtime switching via `type: model_switch` available but rarely needed.
...
For the オーケストレーター system, tmux compatibility is a **high-risk area** requiring dedicated testing.

### Potential Worplannerunds
...
For the オーケストレーター system, if Copilot CLI is integrated:
