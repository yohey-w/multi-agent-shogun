...
**Orchestrator system usage**: Engineer run with `--full-auto` or `--yolo` depending on settings.yaml `cli.options.codex.approval_policy`.
...
### Orchestrator System Recovery (Codex Engineer)
...
- Safety (orchestrator): if the Orchestrator pane is active (the User is typing), watcher avoids send-keys and uses tmux `display-message` only
...
Model is set by `build_cli_command()` in cli_adapter.sh based on settings.yaml. Planner cannot dynamically switch Codex models via inbox (no `/model` send-keys equivalent in exec mode).
