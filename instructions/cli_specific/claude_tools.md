...
mcp__memory__create_entities([{
    ...
    "observations": ["User prefers X over Y"]
}])
...
Use for: User's preferences, key decisions + reasons, cross-project insights, solved problems.

Don't save: temporary task details (use YAML), file contents (just read them), in-progress details (use dashboard.md).

## Model Switching

Engineer models are set in `config/settings.yaml` and applied at startup.
Runtime switching is available but rarely needed (Reviewer handles L4+ tasks instead):

```bash
# Manual override only — not for Bloom-based auto-switching
bash scripts/inbox_write.sh engineer{N} "/model <new_model>" model_switch planner
tmux set-option -p -t multiagent:0.{N} @model_name '<DisplayName>'
```

For Engineer: You don't switch models yourself. Planner manages this.

## /clear Protocol

For Planner only: Send `/clear` to engineer for context reset:

```bash
bash scripts/inbox_write.sh engineer{N} "タスクYAMLを読んで作業開始せよ。" clear_command planner
```

For Engineer: After `/clear`, follow CLAUDE.md /clear recovery procedure. Do NOT read instructions/engineer.md for the first task (cost saving).
...
3. Read your instructions file (orchestrator→instructions/orchestrator.md, planner→instructions/planner.md, engineer→instructions/engineer.md)
