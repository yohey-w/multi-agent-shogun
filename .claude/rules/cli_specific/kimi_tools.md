...
**Orchestrator system usage**: Engineer run with `--yolo` for unattended operation.
...
### Comparison with Orchestrator System

| Aspect | Orchestrator System | Kimi Agent Swarm |
|--------|--------------|-----------------|
| Execution model | tmux panes (separate processes) | In-process (single Python process) |
| Agent count | 10 (orchestrator + planner + 8 engineer) | Up to 100 (claimed) |
...
| Orchestration | Planner (manager agent) | Main agent auto-delegates |

**Key insight**: Kimi's Agent Swarm is complementary, not competing. It could run *inside* a single engineer's tmux pane, providing sub-delegation within that agent.
...
### Orchestrator System Recovery (Kimi Engineer)

```
Step 1: AGENTS.md is auto-loaded (contains recovery procedure)
Step 2: Read queue/tasks/engineer{N}.yaml → determine current task
