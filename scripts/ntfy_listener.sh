...
# ═══════════════════════════════════════════════════════════════
# ntfy Input Listener
# Streams messages from ntfy topic, writes to inbox YAML, wakes orchestrator.
...
while true; do
    ...
    curl -s --no-buffer "${AUTH_ARGS[@]}" "https://ntfy.sh/$TOPIC/json" 2>/dev/null | while IFS= read -r line; do
        ...
        # Auto-reply removed — orchestrator replies directly after processing.

        # Wake orchestrator via inbox (ntfy messages received directly by orchestrator)
        bash "$SCRIPT_DIR/scripts/inbox_write.sh" orchestrator \
            "ntfy new message received. Check queue/ntfy_inbox.yaml and process." \
