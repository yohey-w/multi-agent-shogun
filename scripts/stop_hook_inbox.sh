...
#   2. Auto-notifies planner via inbox_write (background, non-blocking)
...
if [ "$STOP_HOOK_ACTIVE" = "True" ]; then
    ...
    FLAG="${IDLE_FLAG_DIR:-/tmp}/orchestrator_idle_${AGENT_ID}"
    touch "$FLAG"
...
# Orchestrator skips planner notification (orchestrator doesn't report to planner)
...
if [ -n "$LAST_MSG" ]; then
    ...
    # Send notification to planner (background, non-blocking)
    # Orchestrator doesn't report to planner — skip notification
    if [ -n "$NOTIFY_TYPE" ] && [ "$AGENT_ID" != "orchestrator" ]; then
        bash "$SCRIPT_DIR/scripts/inbox_write.sh" planner \
            "$NOTIFY_CONTENT" \
...
if [ "${UNREAD_COUNT:-0}" -eq 0 ]; then
    ...
    # dashboard.md も監視（orchestratorの場合のみ）
    WATCH_TARGETS=("$INBOX")
    if [ "$AGENT_ID" = "orchestrator" ]; then
