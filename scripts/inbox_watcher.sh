...
# Example: bash scripts/inbox_watcher.sh planner multiagent:0.0 claude
#
# 設計思想:
...
if [ "${__INBOX_WATCHER_TESTING__:-}" != "1" ]; then
    ...
    if [[ "$CLI_TYPE" == "claude" ]]; then
        touch "${IDLE_FLAG_DIR:-/tmp}/orchestrator_idle_${AGENT_ID}"
...
disable_normal_nudge() {
    ...
    if [ -f "${IDLE_FLAG_DIR:-/tmp}/orchestrator_idle_${AGENT_ID}" ]; then
...
send_cli_command() {
    ...
    # Safety: never inject CLI commands into the orchestrator pane.
    # Shogun is controlled by the Lord; keystroke injection can clobber human input.
    if [ "$AGENT_ID" = "orchestrator" ]; then
        echo "[$(date)] [SKIP] orchestrator: suppressing CLI command injection ($cmd)" >&2
        return 0
    fi
...
send_context_reset() {
    ...
    # Only engineer should receive automatic context resets (clear stale task context).
    ...
    if [ "$AGENT_ID" = "orchestrator" ] || [ "$AGENT_ID" = "planner" ] || [ "$AGENT_ID" = "reviewer" ]; then
...
agent_is_busy() {
    ...
    if [[ "$effective_cli" == "claude" ]]; then
        ...
        [ ! -f "${IDLE_FLAG_DIR:-/tmp}/orchestrator_idle_${AGENT_ID}" ]
    else
...
#   watching (e.g. single-pane orchestrator session where pane_is_active is always true).
...
send_wakeup() {
    ...
    # Exception: orchestrator — ntfy must be delivered immediately regardless of busy state.
    if agent_is_busy && [[ "$AGENT_ID" != "orchestrator" ]]; then
...
send_wakeup_with_escape() {
    ...
    # Safety: never send Escape escalation to orchestrator. It can wipe the user's input.
    if [ "$AGENT_ID" = "orchestrator" ]; then
        echo "[$(date)] [SKIP] orchestrator: suppressing Escape escalation; sending plain nudge" >&2
...
process_unread() {
    ...
    if no_idle_full_read "$trigger" && [ "$fast_count" -eq 0 ] 2>/dev/null; then
        ...
        touch "${IDLE_FLAG_DIR:-/tmp}/orchestrator_idle_${AGENT_ID}" 2>/dev/null || true
        if ! agent_is_busy; then
            # Shogun: only clear input when pane is not active (Lord is away)
            if [ "$AGENT_ID" = "orchestrator" ] && pane_is_active; then
...
" 2>/dev/null)
    ...
    if [ "$normal_count" -gt 0 ] 2>/dev/null; then
        ...
        # Exception: orchestrator — ntfy must be delivered immediately.
        ...
        #          (3) agent is orchestrator (human-controlled).
        ...
        if [ "$age" -lt "$ESCALATE_PHASE1" ]; then
            ...
            if [ "$LAST_CLEAR_TS" -lt "$((now - ESCALATE_COOLDOWN))" ]; then
                ...
                if [[ "$effective_cli" == "codex" ]]; then
                    ...
                    send_wakeup "$normal_count"
                elif [ "$AGENT_ID" = "orchestrator" ] || [ "$AGENT_ID" = "planner" ] || [ "$AGENT_ID" = "reviewer" ]; then
                    # Command-layer agents (planner/reviewer/orchestrator): suppress /clear even in Phase 3
