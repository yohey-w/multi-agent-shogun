...
#   bash scripts/switch_cli.sh engineer3
#
#   # Codex Spark → Claude Sonnet に切替
#   bash scripts/switch_cli.sh engineer3 --type claude --model claude-sonnet-4-6
#
#   # 同一CLI内でモデルだけ変更（Sonnet → Opus）
#   bash scripts/switch_cli.sh engineer3 --model claude-opus-4-6
#
#   # 全engineerを一括切替
#   for i in $(seq 1 7); do bash scripts/switch_cli.sh engineer$i --type claude --model claude-sonnet-4-6; done
#
# Flow:
...
usage() {
    ...
    echo ""
    echo "  agent_id   planner, engineer1-7, reviewer"
...
resolve_pane() {
    ...
    case "$agent_id" in
        planner)    echo "multiagent:agents.$((pane_base + 0))" ;;
        engineer1)  echo "multiagent:agents.$((pane_base + 1))" ;;
        engineer2)  echo "multiagent:agents.$((pane_base + 2))" ;;
        engineer3)  echo "multiagent:agents.$((pane_base + 3))" ;;
        engineer4)  echo "multiagent:agents.$((pane_base + 4))" ;;
        engineer5)  echo "multiagent:agents.$((pane_base + 5))" ;;
        engineer6)  echo "multiagent:agents.$((pane_base + 6))" ;;
        engineer7)  echo "multiagent:agents.$((pane_base + 7))" ;;
        reviewer)   echo "multiagent:agents.$((pane_base + 8))" ;;
        *)
...
wait_for_shell_prompt() {
    ...
    while [ "$waited" -lt "$max_wait" ]; do
        ...
        # シェルプロンプトの検出パターン
        # PS1にはカスタムプロンプト（start_session由来）や標準的な$/%が含まれる
