...
def handle_orchestrator_to_planner(path: Path, cutoff: int, dry_run: bool) -> FileResult:
    ...
    if not dry_run and archive_items:
        append_yaml_list(
            archive_dir_for_now() / "orchestrator_to_planner.yaml",
            "commands",
            archive_items,
            y,
        )
        res.archive_path = archive_dir_for_now() / "orchestrator_to_planner.yaml"
...
def handle_reviewer_report(path: Path, cutoff: int, dry_run: bool) -> FileResult:
    res = FileResult(path=path)
    if not path.exists():
        res.notes.append("missing — skipped")
        return res
    ...
    if not dry_run and archive:
        archive_path = archive_dir_for_now() / "reviewer_report.yaml"
        archive_path.parent.mkdir(parents=True, exist_ok=True)
...
def handle_single_doc_report(path: Path, cutoff: int, dry_run: bool) -> FileResult:
    ...
    # normalize engineer7-style "completed" → "done"
...
def iter_engineer_report_files() -> Iterable[Path]:
    d = ROOT / "queue" / "reports"
    return sorted(p for p in d.glob("*_report.yaml") if p.name != "reviewer_report.yaml")


def git_commit(cutoff: int, extra_paths: list[str] | None = None) -> str | None:
    paths = ["queue/archive", "queue/orchestrator_to_planner.yaml", "queue/inbox",
...
def run(cutoff_id_str: str, dry_run: bool) -> RunReport:
    ...
    report.results.append(
        handle_orchestrator_to_planner(ROOT / "queue" / "orchestrator_to_planner.yaml", cutoff, dry_run)
    )
    report.results.append(handle_dashboard_md(ROOT / "dashboard.md", cutoff, dry_run))
    report.results.append(
        handle_reviewer_report(ROOT / "queue" / "reports" / "reviewer_report.yaml", cutoff, dry_run)
    )
    ...
    for p in iter_engineer_report_files():
