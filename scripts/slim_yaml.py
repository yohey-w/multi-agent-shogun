...
CANONICAL_TASKS = {f'engineer{i}' for i in range(1, 9)} | {'reviewer'}
CANONICAL_REPORTS = {f'engineer{i}_report' for i in range(1, 9)} | {'reviewer_report'}
...
def get_active_cmd_ids():
    """Return command IDs in orchestrator_to_planner that are not done."""
    queue_dir = get_queue_dir()
    orchestrator_file = queue_dir / 'orchestrator_to_planner.yaml'
    data = load_yaml(orchestrator_file)
...
def slim_orchestrator_to_planner():
    """Archive done/cancelled commands from orchestrator_to_planner.yaml."""
    queue_dir = get_queue_dir()
    archive_dir = queue_dir / 'archive'
    orchestrator_file = queue_dir / 'orchestrator_to_planner.yaml'

    if not orchestrator_file.exists():
        print(f"Warning: {orchestrator_file} not found", file=sys.stderr)
        return True
    ...
    archive_timestamp = get_timestamp()
    archive_file = archive_dir / f'orchestrator_to_planner_{archive_timestamp}.yaml'
    ...
    data[key] = active
    if not save_yaml(orchestrator_file, data):
        print(f"Error: Failed to update {orchestrator_file}, but archive was created", file=sys.stderr)
        return False
...
def main():
    ...
    # Process orchestrator_to_planner if this is planner
    if agent_id == 'planner':
        if not slim_orchestrator_to_planner():
            sys.exit(1)
        migration(dry_run)
