...
### 🌙 11. 夜間稼働モード（caffeinate）

寝る前にコマンドを出して、朝起きたら `dashboard.md` に結果が載っている — それが夜間稼働モード。

**仕組み:**

macOS標準の `caffeinate` ユーティリティがシステムのスリープを防止し、蓋を閉じてもtmuxの全エージェントが稼働し続ける。

```bash
# 起動（システム・ディスプレイ・アイドルスリープを防止）
caffeinate -s -d -i &
...
| エージェント | 挙動 |
|-------------|------|
| 将軍（shogun:0.0） | 殿のターミナルのため、蓋を閉じると入力が止まる |
| 家老・足軽・軍師（multiagent:0.*） | tmuxセッション内で動作し続ける |

`caffeinate` 有効時は蓋を閉じてもtmuxプロセスは生き続ける。家老→足軽→軍師の全パイプラインが自動で完了する。

**運用パターン:**

1. 将軍ペインでコマンドを出す
2. `caffeinate -s -d -i &` を実行
3. 蓋を閉じて就寝
4. 朝、蓋を開けると `dashboard.md` に結果が記載されている

**前提条件:**

- **電源アダプタ接続が必須。** バッテリー駆動では `caffeinate` があっても数時間でスリープに入る可能性あり。
...
> **将来構想:** caffeinate起動 + `DISPLAY_MODE=silent` 設定 + 朝のステータス確認をワンコマンド化するスキルを検討中。
